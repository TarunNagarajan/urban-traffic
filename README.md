...
### Training Log Folders (`training_run_*`)

Every time you run the `train.py` script, a new folder is created in the `logs/` directory with a name like `training_run_YYYYMMDD-HHMMSS`.

-   **Purpose:** These folders contain the raw, detailed simulation output for each individual episode of a training session.
-   **Contents:** Inside, you will find a series of CSV files named `output.csv_conn0_ep*.csv`. Each file corresponds to one episode of the training run.
-   **Use Case:** This data is very granular and is primarily useful for debugging the training process itself. For the main performance analysis and dashboard, you should use the more consolidated data in the `logs/evaluation/` directory.
