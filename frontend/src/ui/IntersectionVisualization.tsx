...
export function IntersectionVisualization({
  ...
  useEffect(() => {
    ...
    ctx.strokeStyle = '#000'; // Black stroke for better contrast
    ctx.lineWidth = 1;

    vehicles.forEach(vehicle => {
      // This scaling is a guess. It might need to be adjusted based on the coordinate range in the data.
      const x = vehicle.x_coord_m * 0.2 + width / 4; 
      const y = vehicle.y_coord_m * 0.2 + height / 2; 

      ctx.beginPath();
      ctx.arc(x, y, 6, 0, 2 * Math.PI); // Increased radius to 6
      ctx.fill();
      ctx.stroke();
    });

  }, [intersections, vehicles, width, height, drawIntersection]);
